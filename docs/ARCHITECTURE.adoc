...
* Output: candidate claim transitions anchored by trigger phrases and
spans.
...
* Output: invariant type, preserved/losses, break condition, and
classification enum.
...
* Input: user command intents (`+scan+`, `+accept+`, `+dismiss+`,
`+clarify+`, `+add+`, `+update+`).
...
* Profiles allow domain-specific defaults without changing core
